growthrate=-0.02;
feedingrateP2=0.00025;
maxfeedingrateP2=0.015;
feedingrateP3=0.00005;
...
for (i=0;i<global.gridWidth;i++){
	for(j=0;j<global.gridHeight;j++){
		if (population4[i,j]>-1){
			diff=global.population4[i,j]*(growthrate+min(feedingrateP2*global.population2[i,j],maxfeedingrateP2)+min(feedingrateP3*global.population3[i,j],maxfeedingrateP3));
			if(diff>0){
				diff=ceil(diff);
			}
			if(diff<0){
