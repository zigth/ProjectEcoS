...
for (i=0;i<global.gridWidth;i++){
	for(j=0;j<global.gridHeight;j++){
		if (population4[i,j]>-1){
			diff=global.population4[i,j]*(growthrate+min(feedingrateP2*global.population2[i,j],maxfeedingrateP2)+min(feedingrateP3*global.population3[i,j],maxfeedingrateP3)+competitionrateP1*global.population4[i,j]/global.population1[i,j]);
			if(diff>0){
				diff=ceil(diff);
			}
			if(diff<0){
